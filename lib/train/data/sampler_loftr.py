    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                elif self.frame_sample_mode == 'rand_RGBT':
                    base_frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=self.num_template_frames - 1,
                                                             max_id=len(visible) - self.num_search_frames)
            ...
            try:
                template_frames,template_anno,search_frames,search_anno,GT = dataset.get_frames(seq_id, template_frame_ids)
                H, W, _ = template_frames[0].shape
                template_masks =[torch.zeros((H, W))] * self.num_template_frames
                search_masks =  [torch.zeros((H, W))] * self.num_search_frames
