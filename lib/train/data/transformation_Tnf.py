...
from .torch_util import expand_dim
...
class ComposedGeometricTnf(object):
    ...
    """
    def __init__(self, tps_grid_size=3, tps_reg_factor=0,  out_h=480, out_w=480,
                 offset_factor=1.0,
...
class GeometricTnf(object):
    ...
    """
    def __init__(self, geometric_model='affine', tps_grid_size=3, tps_reg_factor=0,  out_h=480, out_w=480, offset_factor=None, use_cuda=False):
        self.out_h = out_h
        self.out_w = out_w
        ...
        if geometric_model=='affine' and offset_factor is None:
            ...
            self.theta_identity = torch.Tensor(np.expand_dims(np.array([[1, 0, 0], [0, 1, 0],[0, 0, 1]]), 0).astype(np.float32))
    ...
    def __call__(self, image_batch, theta_batch=None, out_h=None, out_w=None, return_warped_image=True, return_sampling_grid=False, padding_factor=1.0, crop_factor=1.0):
        ...
        if theta_batch is None:
            ...
            if theta_batch.size()[1]==2:
                theta_batch = theta_batch.expand(b,2,3).contiguous()
            elif theta_batch.size()[1]==3:
                theta_batch = theta_batch.view(b,9).contiguous()
        ...
        # sample transformed image
        warped_image_batch = F.grid_sample(image_batch.view(1,c,h,w), sampling_grid)
...
class SynthPairTnf(object):
    ...
    """
    def __init__(self, use_cuda=False, geometric_model='affine', crop_factor=1.0, output_size=(1024,1024), padding_factor = 0.5, occlusion_factor=0):
        ...
        self.rescalingTnf = GeometricTnf(geometric_model, out_h=self.out_h, out_w=self.out_w,
                                         use_cuda = self.use_cuda)
        self.geometricTnf = GeometricTnf(geometric_model, out_h=self.out_h, out_w=self.out_w, 
                                         use_cuda = self.use_cuda)

        
    def __call__(self, frame_template,frame_search, theta_batch):
        """
        ...
        frame_template = torch.from_numpy(frame_template).type(torch.float32)
        frame_search = torch.from_numpy(frame_search).type(torch.float32)
        theta_batch = torch.from_numpy(theta_batch).type(torch.float32)
        #对template图像施加变化

        if self.use_cuda:
            frame_search = frame_search.cuda()
            frame_template= frame_template.cuda()
            theta_batch = theta_batch.cuda()

        #b, c, h, w = frame_template.size() #红外图像的size
        b=1
        '''
        c,h,w=frame_template.shape       
        # 扩展红外图像区域
        frame_template = self.symmetricImagePad(frame_template, self.padding_factor,b,h,w,c)
        '''
        # convert to variables
        # frame_template = Variable(frame_template, requires_grad=False)
        # theta_batch = Variable(theta_batch, requires_grad=False)


        frame_search = self.rescalingTnf(image_batch=frame_search,
                                                theta_batch=None,
                                                padding_factor=self.padding_factor,
                                                crop_factor=self.crop_factor)  # Identity is used as no theta given
        ...
        # get transformed image
        warped_frame_template = self.geometricTnf(image_batch=frame_template,
                                               theta_batch=theta_batch,
                                               padding_factor=self.padding_factor,
                                               crop_factor=self.crop_factor) # Identity is used as no theta given
        #遮挡变化
        if self.occlusion_factor != 0:
            ...
            rolled_indices_1 = torch.LongTensor(np.roll(np.arange(b), 1))
            rolled_indices_2 = torch.LongTensor(np.roll(np.arange(b), 2))
            mask_1 = self.get_occlusion_mask(frame_search.shape, self.occlusion_factor)
            mask_2 = self.get_occlusion_mask(frame_search.shape, self.occlusion_factor)
            ...
            # apply mask
            frame_search = torch.mul(frame_search, 1 - mask_1) + torch.mul(
                frame_search[rolled_indices_1, :], mask_1)
            warped_frame_template = torch.mul(warped_frame_template, 1 - mask_2) + torch.mul(
                warped_frame_template[rolled_indices_1, :], mask_2)
        b1,c1,h1,w1=warped_frame_template.shape
        frame_template_tnf= warped_frame_template.view(-1,h1,w1).numpy().astype(np.uint8)

        b1, c1, h1, w1 = frame_search.shape
        frame_search = frame_search.view(-1, h1, w1).numpy().astype(np.uint8)

        return frame_template_tnf,frame_search,theta_batch.numpy()
        #return {'source_image': rgbimage_batch, 'target_image': warped_frame_template, 'theta_GT': theta_batch}

    def symmetricImagePad(self, image_batch, padding_factor,b,h,w,c):
        #b, c, h, w = frame_template.size()
        ...
        image_batch = torch.cat((image_batch.index_select(1,idx_pad_left),image_batch,
                                 image_batch.index_select(1,idx_pad_right)),1)
...
class SynthTwoStageTnf(SynthPairTnf):
    def __init__(self, use_cuda=True, crop_factor=9/16, output_size=(480,480), padding_factor = 0.5):
...
class SynthTwoPairTnf(SynthPairTnf):
    def __init__(self, use_cuda=True, crop_factor=9/16, output_size=(640,640), padding_factor = 0.5):
        super().__init__(use_cuda=use_cuda)
#        self.aff_reorder_idx=torch.LongTensor([3,2,5,1,0,4])
        self.affTnf = GeometricTnf(geometric_model='affine', out_h=self.out_h, out_w=self.out_w, 
                                         use_cuda = self.use_cuda)
...
class AffineGridGen(Module):
    def __init__(self,  out_h=480, out_w=480, out_ch = 3, use_cuda=True):
    ...
    def forward(self, theta):
        ...
        if not theta.shape==(1,2,3):
            theta = theta.view(1,2,3)
        #theta = theta.contiguous()
        out_size = torch.Size((1,self.out_ch,self.out_h,self.out_w))
        return F.affine_grid(theta, out_size)
    
class AffineGridGenV2(Module):
    def __init__(self,  out_h=480, out_w=480, use_cuda=True):
...
class HomographyGridGen(Module):
    ...
    def forward(self, theta):
        ...
        if theta.size()[0]==1 or theta.size()[0]==9:  # 1 9
            if theta.ndim == 1:
                b = 1
            else:
                b = theta.size()[0]
            if not theta.size() == (b, 9):
                theta = theta.view(b, 9)
                theta = theta.contiguous()
            H = theta
        else:
...
class TpsGridGen(Module):
    def __init__(self, out_h=480, out_w=480, use_regular_grid=True, grid_size=3, reg_factor=0, use_cuda=True):
