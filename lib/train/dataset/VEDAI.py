...
class VEDAI(BaseImageDataset):
    """
    From coco dataset
    """

    def __init__(self, settings,root=None, image_loader=jpeg4py_loader, data_fraction=None, min_area=None,out_h=1024,out_w=1024,
                 split="train"):
        """
        args:
        ...
        root = env_settings().VEDAI_dir if root is None else root
        super().__init__('VEDAI', root, image_loader)
        self.out_h=out_h
        self.out_w=out_h     # same size
        self.settings = settings
        #self.img_pth = os.path.join(root, '{}/'.format(split))
        self.img_pth = os.path.join(root, 'Vehicules512/')

        if split == 'train':
            self.file_path = os.path.join(root, 'Annotations512/fold01.txt')
        elif split == 'val':
            self.file_path = os.path.join(root, 'Annotations512/fold01test.txt')
    ...
    def _get_image_list(self, min_area=None):
        sequence_list = pandas.read_csv(self.file_path, header=None, squeeze=True,dtype='str').values.tolist()
        '''
        root_img=self.img_pth+sequence_list
    ...
    def _get_image(self, im_id,type):
        if type=='search':
            img_list=self.image_list[im_id] #ir
            search_path = img_list + '_ir.png'
            img = self.image_loader(os.path.join(self.img_pth, search_path))
        elif type=='template':
            img_list = self.image_list[im_id]
            template_path=img_list+'_co.png'
    ...
    def get_frames(self, seq_id=None, frame_ids=None, anno=None):
        ...
        frame_template = self._get_image(seq_id,type='template')     # rgb
...
##
    ...
    '''
        ...
        rot_angle = (np.random.rand(1) - 0.5) * 2 * (
                    np.pi / 180 * self.settings.AFF.rot_angle)  # between -np.pi/60 and np.pi/60  60度
        #
        sh_angle = (np.random.rand(1) - 0.5) * 2 * (
                    np.pi / 180 * self.settings.AFF.sh_angle)  # between -np.pi/6 and np.pi/6   18度
        # 拉伸与压缩
