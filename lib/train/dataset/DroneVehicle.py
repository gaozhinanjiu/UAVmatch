...
class DroneVehicle(BaseImageDataset):
    """
    From coco dataset
    """

    def __init__(self, settings, root=None, image_loader=jpeg4py_loader, data_fraction=None, min_area=None, out_h=1024,
                 out_w=1024,
                 split="train"):
        """
        args:
        ...
        root = env_settings().DroneVehicle_dir if root is None else root
    ...
    def _get_image(self, im_id, type):
        if type == 'search':
            path = self.image_list[im_id]  # ir
            img = self.image_loader(os.path.join(self.img_pth, path))
        elif type == 'template':
            path_part = self.image_list[im_id].split('/')
            template_path = 'img/' + path_part[-1]  ###    rgb图像
