...
class STARKS_deformer_Actor(BaseActor):
    ...
    def __init__(self, net, objective, loss_weight, settings):
        ...
        self.loss_grid=TransformedGridLoss(geometric_model='affine', use_cuda=True, grid_size=20)
    ...
    def __call__(self, data):
        ...
        gt_affin = data['affain_gt'].t()  # ( batch, 6)

        # compute losses
        loss, status = self.compute_losses_cnn(out_dict, gt_affin)
    ...
    def forward_pass(self, data, run_box_head, run_cls_head):
        ...
        for i in range(self.settings.num_template):
            ...
            feat_dict_list.append(self.net(img=NestedTensor(template_img_i, template_att_i), mode='backbone_template'))
        ...
        feat_dict_list.append(self.net(img=NestedTensor(search_img, search_att), mode='backbone_search'))
    ...
    def compute_losses_cnn(self, out_dict, gt_affin, return_status=True):
        # Get boxes
        loss_grid=self.loss_grid(out_dict,gt_affin)
        loss_l1=self.L1_loss(out_dict,gt_affin)
        loss = self.loss_weight['grid'] * (loss_grid) + self.loss_weight['l1'] * loss_l1
        if return_status:
            # status for log

            status = {"Loss/total": loss.item(),
                      "Loss/grid": loss_grid.item(),
