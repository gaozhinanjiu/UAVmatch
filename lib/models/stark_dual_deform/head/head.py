...
from lib.models.stark_dual_deform.backbone.backbone import FrozenBatchNorm2d
from lib.models.stark_dual_deform.backbone.repvgg import RepVGGBlock
import math
# import time
...
class FeatureRegression(nn.Module):
    def __init__(self, output_dim=6, hidden_dim=100 ,use_cuda=True, batch_normalization=True, kernel_sizes=[7,5,5], channels=[900,128,64]):
        ...
        # self.conv = nn.Sequential(*nn_modules)       # 卷积层
        #初始化三层MLP网络
        self.linear1 = nn.Linear(hidden_dim*8, output_dim*50)
        self.linear2 = nn.Linear(output_dim*50 , output_dim*10)
        self.linear3 = nn.Linear(output_dim*10 , output_dim)



    def forward(self, x):      #[b,c,hw,1]
...
def build_box_head(cfg):
    if cfg.MODEL.HEAD_TYPE == "MLP":
        hidden_dim = cfg.MODEL.NUM_OBJECT_QUERIES
        #mlp_head = MLP(hidden_dim, hidden_dim, 6, 3,BN=True)  # dim_in, dim_hidden, dim_out, 3 layers
        mlp_head=FeatureRegression(output_dim=cfg.MODEL.TRANSFORMER.TWO_STAGE_NUM_PROPOSALS,hidden_dim=hidden_dim,
                                                   use_cuda=True,
                                                   kernel_sizes=[7,5,20],            #最后一位表示输出的维度
                                                   channels=[256,128,64],
