...
class BANDONDataset(BaseImageDataset):
    """
    From coco dataset
    """
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, data_fraction=None, min_area=None,out_size=1024,
        ...
        root = env_settings().BANDON_dir if root is None else root
        super().__init__('BANDON', root, image_loader)
        self.out_h=out_size
        self.out_w=out_size
        self.img_pth = os.path.join(root, '{}/'.format('train'))
