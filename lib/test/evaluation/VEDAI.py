...
class VEDAIDataset(BaseImageDataset):
    """
    From coco dataset
    """
    ...
    def get_sequence_list(self, min_area=None):
        img_list = pandas.read_csv(self.file_path, header=None, squeeze=True, dtype='str').values.tolist()
        sequence_list=img_list
        if min_area is not None:
            print('error!')
            # image_list = [a for a in image_list if self.coco_set.anns[a]['area'] > min_area]
        for i in range(len(img_list)):
            template_path = img_list[i]+'_co.png'
            sequence_list[i] = os.path.join(self.img_pth, template_path)
        return [['VEDAI',sequence_list,'VEDAI',len(sequence_list)]]
    '''
