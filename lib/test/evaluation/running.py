...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    base_results_path = os.path.join(tracker.results_dir)
...
def run_sequence(dataset_seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    def _results_exist():
        bbox_file = '{}.txt'.format(tracker.results_dir)
    ...
    print('Tracker: {} {} {} '.format(tracker.name, tracker.parameter_name, tracker.run_id))

    if debug:
       AUC_5 = tracker.run_sequence(dataset_seq, debug=debug)
    else:
        try:
            AUC_5 = tracker.run_sequence(dataset_seq, debug=debug)
...
def run_dataset(dataset_name,dataset, trackers, debug=False, threads=0, num_gpus=8):
